...
async function testProbeAndEvaluatorUseResolvedFallbackURL() {
    ...
    global.fetch = async (url, options = {}) => {
        ...
        throw new Error(`Unexpected URL in selector fallback test: ${url}`);
    };
    ...
    try {
        ...
        assert.strictEqual(
            ...
            'probe path should persist the resolved fallback base URL'
        );
        ...
        assert.strictEqual(
            ...
            'http://127.0.0.1:11434',
            'evaluator path should persist the resolved fallback base URL'
        );

        const urls = calls.map((entry) => entry.url);
        assert.ok(urls.includes('http://127.0.0.1:11434/api/generate'), 'probe path should use IPv4 fallback URL');
        assert.ok(urls.includes('http://127.0.0.1:11434/api/chat'), 'evaluator path should use IPv4 fallback URL');
...
async function run() {
    await testProbeAndEvaluatorUseResolvedFallbackURL();
    console.log('ollama-selector-localhost-fallback.test.js: OK');
}

if (require.main === module) {
    run().catch((error) => {
        console.error('ollama-selector-localhost-fallback.test.js: FAILED');
        console.error(error);
        process.exit(1);
    });
}
